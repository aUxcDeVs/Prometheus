...
local function generateName(id, scope)
	-- First try toxic names
	local toxicName = {};
	local d = id % #varNames
	local tempId = (id - d) / #varNames
	table.insert(toxicName, varNames[d + 1]);
	while tempId > 0 do
		local d = tempId % #varNames
		tempId = (tempId - d) / #varNames
		table.insert(toxicName, varNames[d + 1]);
	end
	local toxicResult = table.concat(toxicName, "_");
	
	-- Then append mangled suffix for uniqueness
	local name = ''
